...
test.scores <- get_data('scores', 'greentrike.cfvgdrxonjze.us-west-2.rds.amazonaws.com', 'eotanez', 's4I8$cPPRX5x', 'TEST', 5432)
test.new <- get_data('new', 'greentrike.cfvgdrxonjze.us-west-2.rds.amazonaws.com', 'eotanez', 's4I8$cPPRX5x', 'TEST', 5432)
test.scores
test.new
```
...
```{r}
nn.model <- function(model_or_predict, data){
  if(model_or_predict == 'model'){
    #insert model
    set.seed(3)
...
test.clean.scores.model <- nn.model('model', test.clean.scores)
test.clean.model
```

```{r}

building.predictions <- function(score.model, new.data){
  Scores.predict.new <- predict(score.model, new.data, all.units = FALSE)
...
mainfunction.scores <- function(server, user, password, database, port){
  ...
  score.model <- nn.model('model', clean.scores.data)
...
mainfunction.new <- function(server, user, password, database, port){
  ...
  scores.predict.scores <- nn.model('model', clean.scores.data)
  #then predict with new
  Scores.predict.new <- building.predictions(scores.predict.scores, clean.new.data)
  return(Scores.predict.new)
}
